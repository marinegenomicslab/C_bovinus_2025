#!/usr/bin/env bash
#https://github.com/jpuritz/dDocent/blob/414d6f3f3fba0be75d01da2797629c3808571113/scripts/dDocent_filters

#Usage: dDocent_filters <input.vcf> <output.prefix> <PE or SE> <yes/no (set cutoff)> <If new cutoff, cutoff value>
...
if [[ -z "$2" ]]; then
echo "Usage: `dDocent_filters input.vcf output.prefix <PE or SE> <yes/no (set cutoff)> <If new cutoff, cutoff value>`"
echo "fields in <> are optional"
exit 1
fi
