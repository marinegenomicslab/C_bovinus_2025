...
#Trimming data
cd data
ls -d I* | while read i; do cd $i; cp -s $SLURM_DIR/trim_config.file .; cd ..; done
ls -d I* | while read i; do while [ $(squeue | grep afields | wc -l) -ge 8 ]; do sleep 60; done; cd $i; sbatch $SLURM_DIR/dDocent_trimming.slurm; cd ..; sleep 2; done
...
#Mapping reads
ls | while read i; do cd $i; cp -s ../../data/$i/*.fq.gz .; cp -s ../../reference/Cbovinus_reference.fasta ./reference.fasta; cp -s $SLURM_DIR/map_config.file .; cd ..; done
ls | while read i; do cd $i; sbatch $SLURM_DIR/dDocent_mapping.slurm; cd ..; sleep 2; done

#Filtering bam files
ls | while read i; do cd $i; sbatch $SLURM_DIR/dDocent_bamfiltering.slurm; cd ..; sleep 5; done

#Making coverage files for all data
#Run on normal nodes, scripted to limit to 10 nodes
ls | while read i; do cd $i; sbatch $SLURM_DIR/dDocent_cov.slurm; cd ..; sleep 5; done
...
ls -d *.node | while read i; do 
  echo "checking $i"; 
  cd $i; 
  BEDS=$(ls mapped.*.bed | wc -l); 
  VCF=$(find . -name "*.vcf" -size +62k | wc -l); 
  if [ $VCF -lt $BEDS ]; then 
    echo $i "did not complete all the vcf files properly"; 
  fi; 
  if [ $(find . -name "*.vcf" | wc -l) -gt $BEDS ]; then 
    echo $i "has too many vcf files present"; 
  fi; 
  cd ..; 
done
...
ls -d *.node | while read i; do 
  echo "checking $i"; 
  cd $i; 
  ls raw.*.vcf | while read j; do 
    VCF=$(head -n1 $j| grep "##" | wc -l); 
    if [ $VCF -eq 0 ]; then 
      echo $i $j "missing complete header"; 
      echo ${i}/${j} >> ../bad.vcf; 
    fi; 
  done; 
  cd ..; 
